...
class DemoVisualizer:
    ...
    def store_figs_iter(self, counter: int):
        ...
        # history
        plt.scatter(
            self.particle_positions_hist[counter][:, 0],
            self.particle_positions_hist[counter][:, 1],
        )
        plt.scatter(
            self.global_best_position_hist[counter][0],
            self.global_best_position_hist[counter][1],
        )
    ...
    def generate_gif(self):
        ...
        for ind, filename in enumerate(file_names):
            ...
            if ind != len(file_names) - 1:
                os.remove(filename)
        imageio.mimsave(
            self.benchmark_name / Path("particles.gif"), images, duration=0.1
        )
